...
sum(aggregate(broodstock ~ Spawning.Group, larvae, mean)$broodstock)
...
# Normalize # larvae spawned by #broodstock*mean length - result is # larvae per broodstock centimeter 
...
aggregate(larvae.per.broodcm ~ pH + Temperature + Population, larvae, sum, na.rm=TRUE)                 #By population & treatment
nrow(subset(larvae, total.released >= 50000))                                                       #Number of times >10k larvae were collected (all grps)
...
all_total <- group_by(larvae, Treatment) %>% mutate(cum.total=cumsum(total.released),cum.percap = cumsum(larvae.per.broodcm),CalDay = as.numeric(format(Date,"%j"))) %>% arrange(Date) %>% dplyr::select(Date,CalDay,Treatment,total.released,larvae.per.broodcm,cum.total,cum.percap)
...
for(i in 1:length(metrics)) {
  ...
  qqp(metrics[[i]], "lnorm", main=paste(names(metrics[i]), "-lognormal")) # BEST FIT
  #nbinom <- fitdistr(metrics[[i]], "Negative Binomial")
  #qqp(metrics[[i]], "nbinom", size = nbinom$estimate[[i]], mu = nbinom$estimate[[2]])
  poisson <- fitdistr(metrics[[i]], "Poisson")
  qqp(metrics[[i]], "pois", poisson$estimate, main=paste(names(metrics[i]), "-poisson")) 
}
...
str(spawning_group_sum)   #review summary table, then run stats 

summary(total.released.aov <- aov(log(total.released+1) ~ Temperature*pH, data=spawning_group_total)) # <-- daily release data NO DIFF 
summary(overall_Total.aov <- aov(overall_Total ~ Temperature*pH, data=spawning_group_sum)) # <-- cumulative release NO DIFF 
summary(total.percap.aov <- aov(total.percap ~ Temperature*pH, data=spawning_group_sum)) # <-- cumulative release per oyster*cm NO DIFF 
summary(mean.larvae.aov <- aov(mean.larvae ~ Temperature*pH, data=spawning_group_sum)) # <-- NO DIFF 
summary(cum.total.aov <- aov(log(cum.total+1) ~ Temperature*pH, data=spawning_group_total)) # <-- TEMPERATURE and TEMPERATURE:PH interaction significant (likely due to timing)
summary(cum.percap.aov <- aov(log(cum.percap+1) ~ Temperature*pH, data=spawning_group_total)) # <-- significance holds for the running total per broodstock length 
summary(first.big.aov <- aov(first.big ~ Temperature*pH, data=spawning_group_sum)) # <-- NO DIFF 
summary(max.aov <- aov(max ~ Temperature*pH, data=spawning_group_sum)) # <-- NO DIFF 
summary(maxday.aov <- aov(maxday ~ Temperature*pH, data=spawning_group_sum)) # <-- Temperature effect on max day (no pH interaction, remove)
summary(maxday.aov <- aov(maxday ~ Temperature, data=spawning_group_sum)) # <-- Temperature effect on max day again w/o pH term 
summary(release.days <- aov(release.days ~ Temperature*pH, data=spawning_group_sum)) # <-- NO DIFF 
